...
def ex_9():
    def generate_graph():
        ...
        res = len(find_paths())
        return graph_data, new_graph_data, res, del_node, final_node
    ...
    graph, new_graph, number_of_tracks, del_node, final_node  = generate_graph()
    ...
    hello_9 = f'На рисунке видна карта дорог, связывающих города, направления по которым можно двигаться' \
              f'указаны стрелками. Сколько существует различных путей из города "A" в город {final_node}, ' \
              f'не проходящих через город {del_node}???'
    nx.draw_planar(G, **options, with_labels=True)
    plt.show()
    return hello_9, number_of_tracks
